...
def read_numbers(line): # This function is written to parse the file

    numbers = int(line.split(':')[-1].strip())
    return numbers
        
def read_strings(line): # This function is written to parse the file

    strings = line.split(':')[0].strip()
    return strings

def read_policy(line): # This function is written to parse the file

    policy = line.split(':')[-1].strip()
    return policy

def string_to_int(hex_val): # to convert hexadecimal to integer

    hex_to_int = int("0x"+hex_val, 16)
    return hex_to_int       

def right_shift(arr, string, new_tag):
...
def calc_miss_hit(arr, tag):  # to calculate hit and miss
...
class PageTable():
    ...
    def __init__(self, ):
        ...
        self.virtual_pageno = 0
        self.physical_pageno = 0
        self.pagesize = 0
        self.index = 0
        self.offset = 0
    ...
    def read(self, path_to_config_file):
        ...
        for (string, line) in enumerate(lines):
            ...
            if (line.startswith("Page Table configuration")):
                ...
                self.virtual_pageno = read_numbers(lines[string+1])
                self.physical_pageno = read_numbers(lines[string+2])
                self.pagesize = read_numbers(lines[string+3])

                self.index = int(math.log2(self.virtual_pageno))
                self.offset = int(math.log2(self.pagesize))

                print("Number of virtual pages is "+str(self.virtual_pageno)+".\n")
                print("Number of physical pages is "+str(self.physical_pageno)+".\n")
...
if __name__ == "__main__":
    ...
    path_to_config_file = "/home/pgajjala/COSC530_FA22_pgajjala/Memory_Hierarchy_Simulator/trace.config"

    # Please replace the file with "long_trace.dat" if you want to check the working of the code on long trace file
    
    path_to_dat_file = "/home/pgajjala/COSC530_FA22_pgajjala/Memory_Hierarchy_Simulator/trace.dat" 
